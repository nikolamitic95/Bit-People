    ...
    return (
        <Col className={`${gender === "female" ? 'female' : ''} users__user`} lg={12} md={12} sm={12}>
            ...
            <div className="users__user_info">
                <p className='users__user_fontname'><span> {name} {last}</span></p>
                <p className='users__user_font'><i className="fa fa-envelope"></i> email: <span> {hideEmail(email)}</span></p>
                <p className='users__user_font'><i className="fa fa-birthday-cake"></i> date of birth: <span>{convertDate(dob)}</span></p>
            </div>
        </Col>
    )
}
